...
async def validate_output_node(state: InvoiceExtractionState) -> dict:
    ...
    print(f"\n[validate_output] Validating parsed invoice...")
    ...
    try:
        ...
        print(f"Warning: [validate_output] {len(errors)} validation error(s): {errors[:3]}")
    ...
    print(f"Warning: [validate_output] Accepting with {len(errors)} warning(s)")
    return {
